"""Table 3 — best 모델의 클래스별 진단 분해, 9줄.

열: class_name | precision | recall | count_ratio | near_miss_gt | merge_ratio | miou_match
모두 IoU 0.2 매칭 기준. count_ratio<1=과소검출, merge_ratio>1=병합, near_miss_gt=0<IoU<0.2,
miou_match=매칭쌍 평균 IoU. (ap20_check·near_miss_pix·frag_ratio·fp_* 등 비본문 열은 제외)
출처: best 조합·merge×1 예측 JSON.
"""
import os
import sys
...
class Table3Builder:
    """같은 이미지·클래스 내 예측↔GT IoU 매칭으로 진단 지표를 집계한다."""
    ...
    def build(self):
        ...
        for img_id in tqdm(set(gt_idx) | set(pred_idx), desc="진단 지표"):
